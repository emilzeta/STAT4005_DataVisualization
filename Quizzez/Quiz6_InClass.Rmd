...
I would use a static graph if I wanted to present something specific, for example the distribution of double majors among just the stat majors.
...
ggplot(data = df, aes(x = player, y = ace)) +
  geom_boxplot()
...
server <- function(input, output, session) {
  ...
  df_2sub <- reactive({
    filter(player == input$plotchoice)
    })
  
  box_plot <- reactive({ggplot(data = df_2sub, aes(x = player, y = ace)) +
  geom_boxplot() +
    coord_flip()
  })
